...
fn do_test(data: &[u8]) {
    let tx_result: Result<litecoin::blockdata::transaction::Transaction, _> =
    ...
    match tx_result {
        ...
        Ok(mut tx) => {
            ...
            for input in &mut tx.input {
                input.witness = litecoin::blockdata::witness::Witness::default();
            }
            let no_witness_len = litecoin::consensus::encode::serialize(&tx).len();
