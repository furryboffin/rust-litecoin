...
//!
//! If you find a type defined in `rust-litecoin` that implements `Serialize` and does _not_ have a
...
use litecoin::bip32::{ChildNumber, KeySource, Xpriv, Xpub};
use litecoin::blockdata::locktime::{absolute, relative};
use litecoin::blockdata::witness::Witness;
use litecoin::consensus::encode::deserialize;
use litecoin::hashes::{hash160, ripemd160, sha256, sha256d, Hash};
use litecoin::hex::FromHex;
use litecoin::psbt::raw::{self, Key, Pair, ProprietaryKey};
use litecoin::psbt::{Input, Output, Psbt, PsbtSighashType};
use litecoin::sighash::{EcdsaSighashType, TapSighashType};
use litecoin::taproot::{self, ControlBlock, LeafVersion, TapTree, TaprootBuilder};
use litecoin::{
...
fn serde_regression_address() {
    ...
    let addr = Address::p2pkh(&pk, Network::Litecoin);
