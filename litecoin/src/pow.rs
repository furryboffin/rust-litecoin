...
impl Work {
    ...
    /// used mainly for informative and displaying purposes, similarly to Litecoin Core's
...
///
/// ref: <https://en.litecoin.it/wiki/Target>
#[derive(Copy, Clone, Debug, PartialEq, Eq, PartialOrd, Ord, Hash)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
#[cfg_attr(feature = "serde", serde(crate = "actual_serde"))]
...
impl Target {
    /// When parsing nBits, Litecoin Core converts a negative target threshold into a target of zero.
    ...
    ///
    /// ref: <https://en.litecoin.it/wiki/Target>
    // In Litecoind this is ~(u256)0 >> 32 stored as a floating-point type so it gets truncated, hence
    ...
    // Taken from Litecoin Core but had lossy conversion to/from compact form.
    // https://github.com/litecoin/litecoin/blob/8105bce5b384c72cf08b25b7c5343622754e7337/src/kernel/chainparams.cpp#L208
    pub const MAX_ATTAINABLE_TESTNET: Self = Target(U256(0xFFFF_u128 << (208 - 128), 0));

    /// The proof of work limit on regtest.
    // Taken from Litecoin Core but had lossy conversion to/from compact form.
    // https://github.com/litecoin/litecoin/blob/8105bce5b384c72cf08b25b7c5343622754e7337/src/kernel/chainparams.cpp#L411
    pub const MAX_ATTAINABLE_REGTEST: Self = Target(U256(0x7FFF_FF00u128 << 96, 0));

    /// The proof of work limit on signet.
    // Taken from Litecoin Core but had lossy conversion to/from compact form.
    // https://github.com/litecoin/litecoin/blob/8105bce5b384c72cf08b25b7c5343622754e7337/src/kernel/chainparams.cpp#L348
    pub const MAX_ATTAINABLE_SIGNET: Self = Target(U256(0x0377_ae00 << 80, 0));

    /// Computes the [`Target`] value from a compact representation.
    ///
    /// ref: <https://developer.litecoin.org/reference/block_chain.html#target-nbits>
    ...
    /// defined for the Litecoin network and `current` is the current [target] for this block. As
    ...
    pub fn difficulty(&self, network: Network) -> u128 {
        let max = match network {
            Network::Litecoin => Target::MAX_ATTAINABLE_MAINNET,
...
/// in the original version of Litecoin, likely copying an idea from OpenSSL.
///
/// OpenSSL's bignum (BN) type has an encoding, which is even called "compact" as in litecoin, which
...
impl U256 {
    ...
    /// 2**256 / (x + 1) == ~x / (x + 1) + 1
    ///
    /// (Equation shamelessly stolen from litecoind)
    ...
    fn mul_u64(self, rhs: u64) -> (U256, bool) {
        ...
        for word in &mut split_le {
            ...
            // This will not overflow, for proof see https://github.com/rust-litecoin/rust-litecoin/pull/1496#issuecomment-1365938572
...
mod tests {
    ...
    #[test]
    fn work_log2() {
        // Compare work log2 to historical Litecoin Core values found in Core logs.
