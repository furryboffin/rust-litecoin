...
impl Denomination {
    ...
    fn precision(self) -> i8 {
        match self {
            Denomination::Litecoin => -8,
            Denomination::CentiLitecoin => -6,
            Denomination::MilliLitecoin => -5,
            Denomination::MicroLitecoin => -2,
            Denomination::NanoLitecoin => 1,
            Denomination::PicoLitecoin => 4,
            Denomination::Bit => -2,
    ...
    fn as_str(self) -> &'static str {
        match self {
            Denomination::Litecoin => "BTC",
            Denomination::CentiLitecoin => "cBTC",
            Denomination::MilliLitecoin => "mBTC",
            Denomination::MicroLitecoin => "uBTC",
            Denomination::NanoLitecoin => "nBTC",
            Denomination::PicoLitecoin => "pBTC",
    ...
    fn forms(s: &str) -> Option<Self> {
        match s {
            "BTC" | "btc" => Some(Denomination::Litecoin),
            "cBTC" | "cbtc" => Some(Denomination::CentiLitecoin),
            "mBTC" | "mbtc" => Some(Denomination::MilliLitecoin),
            "uBTC" | "ubtc" => Some(Denomination::MicroLitecoin),
            "nBTC" | "nbtc" => Some(Denomination::NanoLitecoin),
            "pBTC" | "pbtc" => Some(Denomination::PicoLitecoin),
...
/// Amount
///
/// The [Amount] type can be used to express Litecoin amounts that support
...
impl Amount {
    ...
    /// Convert from a value expressing litecoins to an [Amount].
    pub fn from_btc(btc: f64) -> Result<Amount, ParseAmountError> {
        Amount::from_float_in(btc, Denomination::Litecoin)
    }

    /// Convert from a value expressing integer values of litecoins to an [Amount]
    /// in const context.
    ///
    /// ## Panics
    ///
    /// The function panics if the argument multiplied by the number of sats
    /// per litecoin overflows a u64 type.
    ...
    /// Express this [`Amount`] as a floating-point value in Litecoin.
    ///
    /// Please be aware of the risk of using floating-point numbers.
    ///
    /// # Examples
    /// ```
    /// # use litecoin::{Amount, Denomination};
    /// let amount = Amount::from_sat(100_000);
    /// assert_eq!(amount.to_btc(), amount.to_float_in(Denomination::Litecoin))
    /// ```
    pub fn to_btc(self) -> f64 { self.to_float_in(Denomination::Litecoin) }
...
// Just using Litecoin denominated string.
impl fmt::Display for Amount {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        self.fmt_value_in(f, Denomination::Litecoin)?;
        write!(f, " {}", Denomination::Litecoin)
    }
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match &self.style {
            ...
            DisplayStyle::DynamicDenomination if self.sats_abs >= Amount::ONE_BTC.to_sat() => {
                fmt_satoshi_in(self.sats_abs, self.is_negative, f, Denomination::Litecoin, true, format_options)
            },
            DisplayStyle::DynamicDenomination => {
...
/// SignedAmount
///
/// The [SignedAmount] type can be used to express Litecoin amounts that support
/// arithmetic and conversion to various denominations.
///
///
/// Warning!
...
impl SignedAmount {
    ...
    /// Convert from a value expressing litecoins to an [SignedAmount].
    pub fn from_btc(btc: f64) -> Result<SignedAmount, ParseAmountError> {
        SignedAmount::from_float_in(btc, Denomination::Litecoin)
    }
    ...
    /// Express this [`SignedAmount`] as a floating-point value in Litecoin.
    ///
    /// Equivalent to `to_float_in(Denomination::Litecoin)`.
    ///
    /// Please be aware of the risk of using floating-point numbers.
    pub fn to_btc(self) -> f64 { self.to_float_in(Denomination::Litecoin) }
...
pub mod serde {
    ...
    //!     #[serde(with = "litecoin::amount::serde::as_btc")]
    //!     pub amount: Amount,
    //! }
    //! ```
    ...
    impl SerdeAmount for Amount {
        ...
        fn ser_btc<S: Serializer>(self, s: S) -> Result<S::Ok, S::Error> {
            f64::serialize(&self.to_float_in(Denomination::Litecoin), s)
        }
        fn des_btc<'d, D: Deserializer<'d>>(d: D) -> Result<Self, D::Error> {
            use serde::de::Error;
...
mod tests {
    ...
    fn floating_point() {
        ...
        assert_eq!(f(11.22, D::Litecoin), Ok(sat(1122000000)));
        assert_eq!(sf(-11.22, D::MilliLitecoin), Ok(ssat(-1122000)));
        assert_eq!(f(11.22, D::Bit), Ok(sat(1122)));
        assert_eq!(sf(-1000.0, D::MilliSatoshi), Ok(ssat(-1)));
        assert_eq!(f(0.0001234, D::Litecoin), Ok(sat(12340)));
        assert_eq!(sf(-0.00012345, D::Litecoin), Ok(ssat(-12345)));
        ...
        assert_eq!(sf(-100.0, D::MilliSatoshi), Err(ParseAmountError::TooPrecise));
        assert_eq!(f(42.123456781, D::Litecoin), Err(ParseAmountError::TooPrecise));
        assert_eq!(sf(-184467440738.0, D::Litecoin), Err(ParseAmountError::TooBig));
        ...
        assert_eq!(btc(2.5).to_float_in(D::Litecoin), 2.5);
        assert_eq!(btc(-2.5).to_float_in(D::MilliLitecoin), -2500.0);
        ...
        assert_eq!(&btc(0.0012).to_float_in(D::Litecoin).to_string(), "0.0012")
    }

    #[test]
    #[allow(clippy::inconsistent_digit_grouping)] // Group to show 100,000,000 sats per litecoin.
    fn parsing() {
        use super::ParseAmountError as E;
        let btc = Denomination::Litecoin;
        ...
        // exactly 50 chars.
        assert_eq!(
            p("100000000000000.0000000000000000000000000000000000", Denomination::Litecoin),
            Err(E::TooBig)
        );
        // more than 50 chars.
        assert_eq!(
            p("100000000000000.00000000000000000000000000000000000", Denomination::Litecoin),
            Err(E::InputTooLarge)
        );
    }
    ...
    fn to_string() {
        ...
        assert_eq!(Amount::ONE_BTC.to_string_in(D::Litecoin), "1");
        assert_eq!(format!("{:.8}", Amount::ONE_BTC.display_in(D::Litecoin)), "1.00000000");
        assert_eq!(Amount::ONE_BTC.to_string_in(D::Satoshi), "100000000");
        assert_eq!(Amount::ONE_SAT.to_string_in(D::Litecoin), "0.00000001");
        assert_eq!(SignedAmount::from_sat(-42).to_string_in(D::Litecoin), "-0.00000042");

        assert_eq!(Amount::ONE_BTC.to_string_with_denomination(D::Litecoin), "1 BTC");
        ...
        assert_eq!(Amount::ONE_SAT.to_string_with_denomination(D::Litecoin), "0.00000001 BTC");
        assert_eq!(
            SignedAmount::from_sat(-42).to_string_with_denomination(D::Litecoin),
            "-0.00000042 BTC"
        );
    }
    ...
    fn to_from_string_in() {
        ...
        assert_eq!("0.00253583", Amount::from_sat(253583).to_string_in(D::Litecoin));
        assert_eq!("-5", SignedAmount::from_sat(-5).to_string_in(D::Satoshi));
        assert_eq!("0.1", Amount::from_sat(100_000_00).to_string_in(D::Litecoin));
        assert_eq!("-100", SignedAmount::from_sat(-10_000).to_string_in(D::Bit));
        assert_eq!("2535830", Amount::from_sat(253583).to_string_in(D::NanoLitecoin));
        assert_eq!("-100000", SignedAmount::from_sat(-10_000).to_string_in(D::NanoLitecoin));
        assert_eq!("2535830000", Amount::from_sat(253583).to_string_in(D::PicoLitecoin));
        assert_eq!("-100000000", SignedAmount::from_sat(-10_000).to_string_in(D::PicoLitecoin));
        ...
        assert_eq!(
            "0.10000000",
            format!("{:.8}", Amount::from_sat(100_000_00).display_in(D::Litecoin))
        );
        ...
        assert_eq!(ua_str(&ua_sat(500).to_string_in(D::Litecoin), D::Litecoin), Ok(ua_sat(500)));
        assert_eq!(
            ua_str(&ua_sat(21_000_000).to_string_in(D::Bit), D::Bit),
            Ok(ua_sat(21_000_000))
        );
        assert_eq!(
            ua_str(&ua_sat(1).to_string_in(D::MicroLitecoin), D::MicroLitecoin),
            Ok(ua_sat(1))
        );
        assert_eq!(
            ua_str(&ua_sat(1_000_000_000_000).to_string_in(D::MilliLitecoin), D::MilliLitecoin),
            Ok(ua_sat(1_000_000_000_000))
        );
        assert_eq!(
            ua_str(&ua_sat(u64::MAX).to_string_in(D::MilliLitecoin), D::MilliLitecoin),
            Err(ParseAmountError::TooBig)
        );

        assert_eq!(
            sa_str(&sa_sat(-1).to_string_in(D::MicroLitecoin), D::MicroLitecoin),
            Ok(sa_sat(-1))
        );

        assert_eq!(
            sa_str(&sa_sat(i64::MAX).to_string_in(D::Satoshi), D::MicroLitecoin),
            Err(ParseAmountError::TooBig)
        );
        // Test an overflow bug in `abs()`
        assert_eq!(
            sa_str(&sa_sat(i64::MIN).to_string_in(D::Satoshi), D::MicroLitecoin),
            Err(ParseAmountError::TooBig)
        );

        assert_eq!(
            sa_str(&sa_sat(-1).to_string_in(D::NanoLitecoin), D::NanoLitecoin),
            Ok(sa_sat(-1))
        );
        assert_eq!(
            sa_str(&sa_sat(i64::MAX).to_string_in(D::Satoshi), D::NanoLitecoin),
            Err(ParseAmountError::TooPrecise)
        );
        assert_eq!(
            sa_str(&sa_sat(i64::MIN).to_string_in(D::Satoshi), D::NanoLitecoin),
            Err(ParseAmountError::TooPrecise)
        );

        assert_eq!(
            sa_str(&sa_sat(-1).to_string_in(D::PicoLitecoin), D::PicoLitecoin),
            Ok(sa_sat(-1))
        );
        assert_eq!(
            sa_str(&sa_sat(i64::MAX).to_string_in(D::Satoshi), D::PicoLitecoin),
            Err(ParseAmountError::TooPrecise)
        );
        assert_eq!(
            sa_str(&sa_sat(i64::MIN).to_string_in(D::Satoshi), D::PicoLitecoin),
            Err(ParseAmountError::TooPrecise)
        );
    }
    ...
    fn to_string_with_denomination_from_str_roundtrip() {
        ...
        assert_eq!(Amount::from_str(&denom(amt, D::Litecoin)), Ok(amt));
        assert_eq!(Amount::from_str(&denom(amt, D::MilliLitecoin)), Ok(amt));
        assert_eq!(Amount::from_str(&denom(amt, D::MicroLitecoin)), Ok(amt));
        assert_eq!(Amount::from_str(&denom(amt, D::Bit)), Ok(amt));
        assert_eq!(Amount::from_str(&denom(amt, D::Satoshi)), Ok(amt));
        assert_eq!(Amount::from_str(&denom(amt, D::NanoLitecoin)), Ok(amt));
        assert_eq!(Amount::from_str(&denom(amt, D::MilliSatoshi)), Ok(amt));
        assert_eq!(Amount::from_str(&denom(amt, D::PicoLitecoin)), Ok(amt));
