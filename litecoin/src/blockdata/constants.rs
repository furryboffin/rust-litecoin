...
pub const PUBKEY_ADDRESS_PREFIX_MAIN: u8 = 0x30; // 0x30
/// Mainnet (litecoin) script address prefix.
pub const SCRIPT_ADDRESS_PREFIX_MAIN: u8 = 0x32; // 0x32
/// Test (tesnet, signet, regtest) pubkey address prefix.
pub const PUBKEY_ADDRESS_PREFIX_TEST: u8 = 111; // 0x6f
/// Test (tesnet, signet, regtest) script address prefix.
pub const SCRIPT_ADDRESS_PREFIX_TEST: u8 = 0x3a; // 0x3a
...
fn litecoin_genesis_tx() -> Transaction {
    // Base
...
pub fn genesis_block(network: Network) -> Block {
    let txdata = vec![litecoin_genesis_tx()];
    let hash: sha256d::Hash = txdata[0].txid().into();
    let merkle_root = hash.into();
    match network {
        Network::Litecoin => Block {
            header: block::Header {
...
impl ChainHash {
    ...
    /// `ChainHash` for regtest litecoin.
    ...
    pub const fn using_genesis_block(network: Network) -> Self {
        let hashes = [Self::LITECOIN, Self::TESTNET, Self::SIGNET, Self::REGTEST];
...
mod test {
    ...
    #[test]
    fn litecoin_genesis_first_transaction() {
        let gen = litecoin_genesis_tx();
    ...
    fn litecoin_genesis_full_block() {
        let gen = genesis_block(Network::Litecoin);
    ...
    chain_hash_genesis_block! {
        mainnet_chain_hash_genesis_block, Network::Litecoin;
    ...
    fn mainnet_chain_hash_test_vector() {
        let got = ChainHash::using_genesis_block(Network::Litecoin).to_string();
