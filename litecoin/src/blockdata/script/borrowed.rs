...
/// Litecoin script slice.
///
/// *[See also the `litecoin::blockdata::script` module](crate::blockdata::script).*
///
...
///
/// * [CScript definition](https://github.com/litecoin/litecoin/blob/d492dc1cdaabdc52b0766bf4cba4bd73178325d0/src/script/script.h#L410)
///
...
impl Script {
    ...
    ///
    /// [BIP143]: <https://github.com/litecoin/bips/blob/99701f68a88ce33b2d0838eb84e115cef505b4c2/bip-0143.mediawiki>
    ...
    /// broadcastable on today's Litecoin network.
    pub fn dust_value(&self) -> crate::Amount {
        // This must never be lower than Litecoin Core's GetDustThreshold() (as of v0.21) as it may
    ...
    ///
    /// In Litecoin Core, there are two ways to count sigops, "accurate" and "legacy".
    ...
    ///
    /// Litecoin Core uses accurate counting for sigops contained within redeemScripts (P2SH)
    ...
    ///
    /// Litecoin Core uses legacy counting for sigops contained within scriptSigs and
    ...
    fn count_sigops_internal(&self, accurate: bool) -> usize {
        ...
        for inst in self.instructions() {
            match inst {
                    ...
                    match opcode {
                        ...
                        OP_CHECKMULTISIG | OP_CHECKMULTISIGVERIFY => {
                            match (accurate, pushnum_cache) {
                                ...
                                _ => {
                                    // MAX_PUBKEYS_PER_MULTISIG from Litecoin Core
                                    // https://github.com/litecoin/litecoin/blob/v25.0/src/script/script.h#L29-L30
                                    n += 20;
                                }
                            }
                        }
                ...
                // In Litecoin Core it does `if (!GetOp(pc, opcode)) break;`
                Err(_) => break,
            }
        }
    ...
    pub(crate) fn last_pushdata(&self) -> Option<Push> {
        match self.instructions().last() {
            ...
            // OP_16 (0x60) and lower are considered "pushes" by Litecoin Core (excl. OP_RESERVED).
...
///
/// "push" opcodes are defined by Litecoin Core as OP_PUSHBYTES_, OP_PUSHDATA, OP_PUSHNUM_, and
