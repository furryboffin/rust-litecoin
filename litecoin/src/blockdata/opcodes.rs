...
//! Litecoin script opcodes.
//!
//! Litecoin's script uses a stack-based assembly language. This module defines
...
///
///   Litecoin Core's `IsPushOnly` considers `OP_RESERVED` to be a "push code", allowing this opcode
...
macro_rules! all_opcodes {
    ($($op:ident => $val:expr, $doc:expr);*) => {
        ...
        ///
        /// The `all` module is provided so one can use a wildcard import `use litecoin::opcodes::all::*` to
...
all_opcodes! {
    ...
    OP_CHECKSIG => 0xac, "<https://en.litecoin.it/wiki/OP_CHECKSIG> pushing 1/0 for success/failure.";
    OP_CHECKSIGVERIFY => 0xad, "<https://en.litecoin.it/wiki/OP_CHECKSIG> returning success/failure.";
    ...
    OP_CLTV => 0xb1, "<https://github.com/litecoin/bips/blob/master/bip-0065.mediawiki>";
    OP_CSV => 0xb2, "<https://github.com/litecoin/bips/blob/master/bip-0112.mediawiki>";
