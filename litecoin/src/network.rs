...
//! Litecoin network we are operating on e.g., signet, regtest. The terms
...
pub enum Network {
    /// Mainnet Litecoin.
    Litecoin,
    /// Litecoin's testnet network.
    Testnet,
    /// Litecoin's signet network.
    Signet,
    /// Litecoin's regtest network.
    Regtest,
}
...
impl Network {
    ...
    ///
    /// ```rust
    /// use litecoin::p2p::Magic;
    /// use litecoin::Network;
    /// use std::convert::TryFrom;
    ///
    /// assert_eq!(Ok(Network::Litecoin), Network::try_from(Magic::from_bytes([0xF9, 0xBE, 0xB4, 0xD9])));
    ...
    /// Converts a `Network` to its equivalent `litecoind -chain` argument name.
    ///
    /// ```bash
    /// $ litecoin-23.0/bin/litecoind --help | grep -C 3 '\-chain=<chain>'
    ...
    /// Converts a `litecoind -chain` argument name to its equivalent `Network`.
    ///
    /// ```bash
    /// $ litecoin-23.0/bin/litecoind --help | grep -C 3 '\-chain=<chain>'
    /// Chain selection options:
    ...
    /// ```rust
    /// use litecoin::Network;
    /// use litecoin::blockdata::constants::ChainHash;
    ///
    /// let network = Network::Litecoin;
    /// assert_eq!(network.chain_hash(), ChainHash::LITECOIN);
    /// ```
    ...
    /// use std::convert::TryFrom;
    ///
    /// assert_eq!(Ok(Network::Litecoin), Network::try_from(ChainHash::LITECOIN));
    /// ```
...
#[cfg(feature = "serde")]
pub mod as_core_arg {
    //! Module for serialization/deserialization of network variants into/from Litecoin Core values
    #![allow(missing_docs)]
            ...
            fn visit_str<E: serde::de::Error>(self, s: &str) -> Result<Self::Value, E> {
                Network::from_core_arg(s).map_err(|_| {
                    E::invalid_value(
                        ...
                        &"litecoin network encoded as a string (either main, test, signet or regtest)",
                    )
                })
            }
...
impl TryFrom<ChainHash> for Network {
    ...
    fn try_from(chain_hash: ChainHash) -> Result<Self, Self::Error> {
        match chain_hash {
            ...
            ChainHash::LITECOIN => Ok(Network::Litecoin),
...
mod tests {
    ...
    #[test]
    fn serialize_test() {
        assert_eq!(serialize(&Network::Litecoin.magic()), &[0xf9, 0xbe, 0xb4, 0xd9]);
        ...
        assert_eq!(deserialize(&[0xf9, 0xbe, 0xb4, 0xd9]).ok(), Some(Network::Litecoin.magic()));
