...
//! Litecoin addresses.
//!
//! Support for ordinary base58 Litecoin addresses and private keys.
//!
...
//! let address = Address::p2pkh(&public_key, Network::Litecoin);
//! # }
//! ```
//!
...
//!
//! ```toml
//! litecoin = { version = "...", features = ["rand-std"] }
//! ```
...
/// let address: Address<NetworkChecked> = address.require_network(Network::Litecoin).unwrap();
///
/// // variant 2
/// let address: Address = Address::from_str("32iVBEu4dxkUQk9dJbZUiBiQdmypcEyJRf").unwrap()
///                .require_network(Network::Litecoin).unwrap();
///
/// // variant 3
/// let address: Address<NetworkChecked> = "32iVBEu4dxkUQk9dJbZUiBiQdmypcEyJRf".parse::<Address<_>>()
///                .unwrap().require_network(Network::Litecoin).unwrap();
/// ```
///
...
/// ### Relevant BIPs
///
/// * [BIP13 - Address Format for pay-to-script-hash](https://github.com/litecoin/bips/blob/master/bip-0013.mediawiki)
/// * [BIP16 - Pay to Script Hash](https://github.com/litecoin/bips/blob/master/bip-0016.mediawiki)
/// * [BIP141 - Segregated Witness (Consensus layer)](https://github.com/litecoin/bips/blob/master/bip-0141.mediawiki)
/// * [BIP142 - Address Format for Segregated Witness](https://github.com/litecoin/bips/blob/master/bip-0142.mediawiki)
/// * [BIP341 - Taproot: SegWit version 1 spending rules](https://github.com/litecoin/bips/blob/master/bip-0341.mediawiki)
/// * [BIP350 - Bech32m format for v1+ witness addresses](https://github.com/litecoin/bips/blob/master/bip-0350.mediawiki)
#[derive(Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
///
...
#[cfg(feature = "serde")]
crate::serde_utils::serde_string_deserialize_impl!(Address<NetworkUnchecked>, "a Litecoin address");
...
impl Address {
    ...
    /// Checks whether or not the address is following Litecoin standardness rules when
...
impl Address<NetworkUnchecked> {
    ...
    ///
    /// assert_eq!(address.is_valid_for_network(Network::Litecoin), false);
    ///
    /// let address: Address<NetworkUnchecked> = "32iVBEu4dxkUQk9dJbZUiBiQdmypcEyJRf".parse().unwrap();
    /// assert!(address.is_valid_for_network(Network::Litecoin));
    ...
    pub fn is_valid_for_network(&self, network: Network) -> bool {
        ...
        match (self.network(), network) {
            (a, b) if *a == b => true,
            (Network::Litecoin, _) | (_, Network::Litecoin) => false,
...
impl FromStr for Address<NetworkUnchecked> {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let bech32_network = match find_bech32_prefix(s) {
            ...
            "ltcrt" | "LTCRT" => Some(Network::Regtest),
            _ => None,
        };
        ...
        let (network, payload) = match data[0] {
            PUBKEY_ADDRESS_PREFIX_MAIN =>
                (Network::Litecoin, Payload::PubkeyHash(PubkeyHash::from_slice(&data[1..]).unwrap())),
            SCRIPT_ADDRESS_PREFIX_MAIN =>
                (Network::Litecoin, Payload::ScriptHash(ScriptHash::from_slice(&data[1..]).unwrap())),
...
mod tests {
    ...
    use crate::network::Network::{Litecoin, Testnet};
    ...
    fn test_p2pkh_from_key() {
        ...
        let addr = Address::p2pkh(&key, Litecoin);
    ...
    #[test]
    fn test_p2wpkh() {
        // stolen from Litecoin transaction: 1bb95946ebc0c360718563364792b11eda2af3171777c5044c574276ebe7f264
        let mut key = "03b75a697ec533bfde16ce2c68e2e9f23d8754bf56a50cea2b72e4440f1c03c4f0"
            .parse::<PublicKey>()
            .unwrap();
        let addr = Address::p2wpkh(&key, Litecoin).unwrap();
        assert_eq!(&addr.to_string(), "ltc1qscmsckqltx9k9htdveqyet7ahh79f5j3dcmxld");
        assert_eq!(addr.address_type(), Some(AddressType::P2wpkh));
        roundtrips(&addr);

        // Test uncompressed pubkey
        key.compressed = false;
        assert_eq!(Address::p2wpkh(&key, Litecoin), Err(Error::UncompressedPubkey));
    }

    #[test]
    fn test_p2wsh() {
        // stolen from Litecoin transaction 5df912fda4becb1c29e928bec8d64d93e9ba8efa9b5b405bd683c86fd2c65667
        ...
        let addr = Address::p2wsh(&script, Litecoin);
        assert_eq!(
            &addr.to_string(),
    ...
    #[test]
    fn test_p2shwpkh() {
        // stolen from Litecoin transaction: ad3fd9c6b52e752ba21425435ff3dd361d6ac271531fc1d2144843a9f550ad01
        // let mut key = "026c468be64d22761c30cd2f12cbc7de255d592d7904b1bab07236897cc4c2e766"
        // let mut key = "02d53a640d1d5a502b777f4bc10ebce06add7185256ca1f93e60d6cc5e78cebba2"
        let mut key = "03ca16a782c2d057d19d4817a44cb492eaa992e18fa68d3735eef665845705d271"
            .parse::<PublicKey>()
            .unwrap();
        let addr = Address::p2shwpkh(&key, Litecoin).unwrap();
        // assert_eq!(&addr.to_string(), "MEM8TJpPcyMSAxzeW1iJfbbhPkbYKsPENj");
        assert_eq!(&addr.to_string(), "MPY8W6aRrz2yd7DAEMbnkc8vYygGC8DcPw");
        assert_eq!(addr.address_type(), Some(AddressType::P2sh));
        roundtrips(&addr);
    ...
    #[test]
    fn test_p2shwsh() {
        // stolen from Litecoin transaction f9ee2be4df05041d0e0a35d7caa3157495ca4f93b233234c9967b6901dacf7a9
        ...
        let addr = Address::p2shwsh(&script, Litecoin);
    ...
    fn test_non_existent_segwit_version() {
        ...
        let addr = Address::new(Litecoin, Payload::WitnessProgram(witness_prog));
        roundtrips(&addr);
    }
    ...
    fn test_address_type() {
        ...
        for (address, expected_type) in &addresses {
            let addr = Address::from_str(address)
                .unwrap()
                .require_network(Network::Litecoin)
                .expect("mainnet");
    ...
    fn test_qr_string() {
        ...
        {
            let addr =
                Address::from_str(el).unwrap().require_network(Network::Litecoin).expect("mainnet");
    ...
    fn test_valid_networks() {
        ...
        const LEGACY_EQUIVALENCE_CLASSES: &[&[Network]] =
            &[&[Network::Litecoin], &[Network::Testnet, Network::Regtest, Network::Signet]];
        const SEGWIT_EQUIVALENCE_CLASSES: &[&[Network]] =
            &[&[Network::Litecoin], &[Network::Regtest], &[Network::Testnet, Network::Signet]];
    ...
    fn p2tr_from_untweaked() {
        ...
        let address = Address::p2tr(&secp, internal_key, None, Network::Litecoin);
        assert_eq!(
            address.to_string(),
            "bc1p5cyxnuxmeuwuvkwfem96lqzszd02n6xdcjrs20cac6yqjjwudpxqkedrcr"
        );
    ...
    fn test_is_related_to_pubkey_p2tr() {
        ...
        let address = Address::p2tr_tweaked(tweaked_pubkey, Network::Litecoin);
    ...
    fn test_fail_address_from_script() {
        ...
        assert_eq!(Address::from_script(&bad_p2wpkh, Network::Litecoin), expected);
        assert_eq!(Address::from_script(&bad_p2wsh, Network::Litecoin), expected);
        assert_eq!(
            Address::from_script(&invalid_segwitv0_script, Network::Litecoin),
    ...
    fn test_matches_script_pubkey() {
        ...
        for addr in &addresses {
            let addr = Address::from_str(addr).unwrap().require_network(Network::Litecoin).unwrap();
            for another in &addresses {
                let another =
                    Address::from_str(another).unwrap().require_network(Network::Litecoin).unwrap();
