...
//    unusual versioning used in `bitcoinconsensus` to users of `rust-litecoin`.
// 2. We want to implement `std::error::Error` if the "std" feature is enabled in `rust-litecoin` but
//    not in `bitcoinconsensus`.
#[derive(Debug, Clone, PartialEq, Eq)]
#[non_exhaustive]
pub struct LitecoinConsensusError(bitcoinconsensus::Error);

impl fmt::Display for LitecoinConsensusError {
...
impl std::error::Error for LitecoinConsensusError {
...
impl From<bitcoinconsensus::Error> for LitecoinConsensusError {
...
pub enum TxVerifyError {
    ...
    ScriptVerification(LitecoinConsensusError),
...
impl From<LitecoinConsensusError> for TxVerifyError {
    fn from(e: LitecoinConsensusError) -> Self { TxVerifyError::ScriptVerification(e) }
}
