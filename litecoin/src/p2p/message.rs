...
//! Litecoin network messages.
//!
//! This module defines the `NetworkMessage` and `RawNetworkMessage` types that
//! are used for (de)serializing Litecoin objects for transmission on the network.
//!
...
/// [Litecoin Wiki: Protocol Specification](https://en.litecoin.it/wiki/Protocol_specification)
...
mod test {
    ...
    fn serialize_verack_test() {
        assert_eq!(serialize(&RawNetworkMessage::new(Magic::from(Network::Litecoin), NetworkMessage::Verack)),
    ...
    fn serialize_ping_test() {
        assert_eq!(serialize(&RawNetworkMessage::new(Magic::from(Network::Litecoin), NetworkMessage::Ping(100))),
    ...
    fn serialize_mempool_test() {
        assert_eq!(serialize(&RawNetworkMessage::new(Magic::from(Network::Litecoin), NetworkMessage::MemPool)),
    ...
    fn serialize_getaddr_test() {
        assert_eq!(serialize(&RawNetworkMessage::new(Magic::from(Network::Litecoin), NetworkMessage::GetAddr)),
    ...
    fn deserialize_getaddr_test() {
        ...
        let preimage =
            RawNetworkMessage::new(Magic::from(Network::Litecoin), NetworkMessage::GetAddr);
        assert!(msg.is_ok());
    ...
    fn deserialize_version_test() {
        ...
        assert!(msg.is_ok());
        let msg = msg.unwrap();
        assert_eq!(msg.magic, Magic::from(Network::Litecoin));
