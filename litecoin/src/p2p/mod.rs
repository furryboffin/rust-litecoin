...
//! Litecoin p2p network types.
//!
//! This module defines support for (de)serialization and network transport
//! of Litecoin data and Litecoin p2p network messages.
...
impl ServiceFlags {
    ...
    /// set by all Litecoin Core non pruned nodes, and is unset by SPV clients or other light
    /// clients.
    pub const NETWORK: ServiceFlags = ServiceFlags(1 << 0);

    /// GETUTXO means the node is capable of responding to the getutxo protocol request.  Litecoin
    /// Core does not support this but a patch set called Litecoin XT does.
    /// See BIP 64 for details on how this is implemented.
    pub const GETUTXO: ServiceFlags = ServiceFlags(1 << 1);

    /// BLOOM means the node is capable and willing to handle bloom-filtered connections.  Litecoin
...
impl Magic {
    ...
    pub const LITECOIN: Self = Self([0xF9, 0xBE, 0xB4, 0xD9]);
...
impl From<Network> for Magic {
    fn from(network: Network) -> Magic {
        match network {
            ...
            Network::Litecoin => Magic::LITECOIN,
...
impl TryFrom<Magic> for Network {
    ...
    fn try_from(magic: Magic) -> Result<Self, Self::Error> {
        match magic {
            ...
            Magic::LITECOIN => Ok(Network::Litecoin),
