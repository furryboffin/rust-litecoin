...
use litecoin::consensus::{encode, Decodable};
use litecoin::p2p::{self, address, message, message_network};
use litecoin::secp256k1;
use litecoin::secp256k1::rand::Rng;

fn main() {
    // This example establishes a connection to a Litecoin node, sends the intial
    ...
    let first_message =
        message::RawNetworkMessage::new(litecoin::Network::Litecoin.magic(), version_message);
            ...
            match reply.payload() {
                message::NetworkMessage::Version(_) => {
                    ...
                    let second_message = message::RawNetworkMessage::new(
                        litecoin::Network::Litecoin.magic(),
...
fn build_version_message(address: SocketAddr) -> message::NetworkMessage {
    // Building version message, see https://en.litecoin.it/wiki/Protocol_documentation#version
