...
//!
//! You can verify the workflow using `litecoind` and `litecoin-cli`.
//!
//! ## Example Setup
//!
//! 1. Start Litecoin Core in Regtest mode, for example:
//!
//!    `litecoind -regtest -server -daemon -fallbackfee=0.0002 -rpcuser=admin -rpcpassword=pass -rpcallowip=127.0.0.1/0 -rpcbind=127.0.0.1 -blockfilterindex=1 -peerblockfilters=1`
//!
//! 2. Define a shell alias to `litecoin-cli`, for example:
//!
//!    `alias bt=litecoin-cli -rpcuser=admin -rpcpassword=pass -rpcport=18443`
//!
...
use litecoin::bip32::{ChildNumber, DerivationPath, Fingerprint, IntoDerivationPath, Xpriv, Xpub};
use litecoin::consensus::encode;
use litecoin::locktime::absolute;
use litecoin::psbt::{self, Input, Psbt, PsbtSighashType};
use litecoin::secp256k1::{Secp256k1, Signing, Verification};
use litecoin::{
...
impl WatchOnly {
    ...
    ///
    /// A watch-only wallet would typically be online and connected to the Litecoin network. We
    /// 'import' into the wallet the `account_0_xpub` and `master_fingerprint`.
    ///
    /// The reason for importing the `input_xpub` is so one can use litecoind to grab a valid input
