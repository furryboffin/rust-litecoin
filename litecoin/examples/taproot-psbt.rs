...
// We use the alias `alias bt='litecoin-cli -regtest'` for brevity.

// Step 0 - Wipe the `regtest` data directory to start from a clean slate.

// Step 1 - Run `litecoind -regtest -daemon` to start the daemon. Litecoin Core 23.0+ is required.
...
//                  alias bt-benefactor='litecoin-cli -regtest -rpcwallet=benefactor'
//                  alias bt-beneficiary='litecoin-cli -regtest -rpcwallet=beneficiary'
//
...
use litecoin::bip32::{ChildNumber, DerivationPath, Fingerprint, Xpriv, Xpub};
use litecoin::consensus::encode;
use litecoin::hashes::Hash;
use litecoin::key::{TapTweak, XOnlyPublicKey};
use litecoin::opcodes::all::{OP_CHECKSIG, OP_CLTV, OP_DROP};
use litecoin::psbt::{self, Input, Output, Psbt, PsbtSighashType};
use litecoin::secp256k1::Secp256k1;
use litecoin::sighash::{self, SighashCache, TapSighash, TapSighashType};
use litecoin::taproot::{self, LeafVersion, TapLeafHash, TaprootBuilder, TaprootSpendInfo};
use litecoin::{
...
fn generate_bip86_key_spend_tx(
    ...
    let tx1 = Transaction {
        ...
        input: vec![TxIn {
            ...
            sequence: litecoin::Sequence(0xFFFFFFFF), // Ignore nSequence.
    ...
    fn create_inheritance_funding_tx(
        ...
        let next_tx = Transaction {
            ...
            input: vec![TxIn {
                ...
                script_sig: ScriptBuf::new(),
                sequence: litecoin::Sequence(0xFFFFFFFD), // enable locktime and opt-in RBF
                witness: Witness::default(),
            }],
            output: vec![],
        };
...
// Lifted and modified from BDK at https://github.com/litecoindevkit/bdk/blob/8fbe40a9181cc9e22cabfc04d57dac5d459da87d/src/wallet/signer.rs#L469-L503

// Litecoin Dev Kit
// Written in 2020 by Alekos Filini <alekos.filini@gmail.com>
//
// Copyright (c) 2020-2021 Litecoin Dev Kit Developers
//
