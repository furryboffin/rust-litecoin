...
then
    ...
    # in one crate and not in another (e.g. upgrade litecoin_hashes in litecoin but not in secp).
    duplicate_dependencies=$(
        ...
        #
        # Whitelist `litecoin_hashes` while we release it and until secp v0.28.0 comes out.
        cargo tree  --target=all --all-features --duplicates \
            | grep '^[0-9A-Za-z]' \
            | grep -v 'syn' \
            | grep -v 'litecoin_hashes' \
            | wc -l
                          )
...
then
    ...
    cd dep_test
    echo 'litecoin = { path = "..", features = ["serde"] }\n\n' >> Cargo.toml
    # Adding an empty workspace section excludes this crate from the rust-litecoin workspace.
    echo '[workspace]\n\n' >> Cargo.toml
